...
class PaletteGeneratorMaster with Diagnosticable {
  ...
  void _selectSwatches() {
    ...
    for (final PaletteTargetMaster target in allTargets) {
      ...
      final PaletteColorMaster? targetScore = _generateScoredTarget(
        target,
        usedColors,
      );
      if (targetScore != null) {
  ...
  void _generateHarmonyColors() {
    if (dominantColor != null) {
      _harmonyColors = ColorHarmonyMaster.generateHarmonyColors(
        dominantColor!.color,
      );
    }
  }
  ...
  PaletteColorMaster? _generateScoredTarget(
    PaletteTargetMaster target,
    Set<Color> usedColors,
  ) {
    final PaletteColorMaster? maxScoreSwatch = _getMaxScoredSwatchForTarget(
      target,
      usedColors,
    );
  ...
  bool _shouldBeScoredForTarget(
    PaletteColorMaster paletteColor,
    PaletteTargetMaster target,
    Set<Color> usedColors,
  ) {
  ...
  void debugFillProperties(DiagnosticPropertiesBuilder properties) {
    ...
    properties.add(
      IterableProperty<PaletteColorMaster>(
        'paletteColors',
        paletteColors,
    ...
    properties.add(
      IterableProperty<PaletteTargetMaster>(
        'targets',
        targets,
    ...
    if (sourceImageInfo != null) {
      properties.add(
        DiagnosticsProperty<ImageInfoMaster>(
          'sourceImageInfo',
          sourceImageInfo,
        ),
      );
